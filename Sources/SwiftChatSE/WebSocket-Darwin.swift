//
//  WebSocket-Darwin.swift
//  SwiftChatSE
//
//  Created by NobodyNada on 7/14/18.
//
...
// Extension for compatibility with the LWS implementation

private let wsEventQueue = DispatchQueue(label: "com.nobodynada.FireAlarm.WebSocket-Darwin.eventQueue", attributes: .concurrent)
...
public extension WebSocket {
    ...
    public func onOpen(_ handler: ((WebSocket) -> Void)?) {
        self.binaryType = .nsData
        event.open = { [weak self] in self.map { handler?($0) } }
    }
    
    private func registerMessageHandler() {
        event.message = { [weak self] message in
            guard let s = self else { return }
            
            if let text = message as? String {
                s.textHandler?(s, text)
            } else if let binary = message as? Data {
                s.dataHandler?(s, binary)
            } else {
                fatalError("invalid message type: \(message)")
            }
        }
    }
    ...
    public func onError(_ handler: ((WebSocket) -> Void)?) {
        event.error = { [weak self] error in self.map {
            $0.error = error
            handler?($0)
            }
        }
    ...
    public static func open(_ url: URL, origin: String? = nil, headers: String? = nil) throws -> WebSocket {
        ...
        var headerDict = (headers?
            ...
            .filter { !$0.isEmpty }
            .map { (header: String) -> (String, String) in
                let components = header.components(separatedBy: ": ")
                precondition(components.count >= 2, "Invalid HTTP header: \(header)")
                
                let key = components.first!
                let value = components.dropFirst().joined(separator: ": ")
                return (key, value)
            }
            ).flatMap { Dictionary($0, uniquingKeysWith: { [$0, $1].joined(separator: ",") }) } ?? [:]
        
        if let origin = origin { headerDict["Origin"] = origin }
 ...
 * of the MIT license.  See the LICENSE-SwiftWebSocket file for details.
 *
 */
...
open class WebSocket: NSObject {
    //Added by SwiftChatSE for compatibility with the LWS implementation
    fileprivate var textHandler: ((WebSocket, String) -> ())?
    fileprivate var dataHandler: ((WebSocket, Data) -> ())?
